...
import { 
  CalendarCheck, Banknote, X, CheckSquare, Filter, Clock, Settings, GripVertical, Navigation, ChevronDown, ChevronUp, Store
} from 'lucide-react';
...
export const SchedulePage: React.FC<SchedulePageProps> = ({
  ...
  const [isWalkInAccordionOpen, setIsWalkInAccordionOpen] = useState(true);
  ...
  const { scheduleOrders, timelineOrders, walkInOrders } = useMemo(() => {
    ...
    uniqueOrders.forEach(order => {
      ...
      if (isWalkInCustomer(order.paymentTerm || customer?.paymentTerm)) {
        walkIn.push(order);
      } else {
        timeline.push(order);
      }
    ...
    return { 
      scheduleOrders: uniqueOrders, 
      timelineOrders: timeline, 
      walkInOrders: walkIn 
    };
      ...
      <div className="px-1">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4, delay: 0.2 }} className="bg-slate-700 rounded-[28px] p-5 shadow-lg text-white mb-6 relative overflow-hidden">
          ...
          <div className="w-full bg-slate-600 rounded-full h-1.5 mt-2 relative z-10">
            <motion.div className="bg-emerald-400 h-1.5 rounded-full" initial={{ width: 0 }} animate={{ width: `${scheduleMoneySummary.totalReceivable > 0 ? (scheduleMoneySummary.totalCollected / scheduleMoneySummary.totalReceivable) * 100 : 0}%` }} transition={{ duration: 1, ease: "easeOut" }} />
          </div>
          <p className="text-[9px] text-slate-400 mt-2 text-right relative z-10 tracking-wide">尚有 ${(scheduleMoneySummary.totalReceivable - scheduleMoneySummary.totalCollected).toLocaleString()} 未收</p>
        </motion.div>
        ...
        <div className="space-y-6 pb-20">
          <div className="flex justify-between items-center px-2">
            ...
            <div className="flex items-center gap-2">
              <div className="text-xs font-bold text-gray-300 tracking-wide">
                共 {scheduleOrders.length} 筆（固定 {timelineOrders.length} / 散客 {walkInOrders.length}）
              </div>
              <button 
          ...
          {(() => {
            ...
            // 如果主時間軸無訂單但有散客訂單，提示固定店家無單
            if (timelineOrders.length === 0 && walkInOrders.length > 0) {
              return (
                <div className="bg-slate-50 border border-dashed border-slate-200 rounded-2xl p-6 text-center">
                  <p className="text-xs text-slate-400 font-medium">本日無固定店家配送排程，僅有散客/現場自取訂單</p>
                </div>
              );
            }
          ...
          {/* CHANGED: 2. 散客 / 現場自取區（手風琴收合卡片） */}
          {walkInOrders.length > 0 && (
            <div className="bg-slate-50/90 border border-slate-200/80 rounded-2xl p-4 shadow-sm">
              <div 
                className="flex items-center justify-between font-bold text-slate-700 cursor-pointer select-none py-1"
                onClick={() => setIsWalkInAccordionOpen(!isWalkInAccordionOpen)}
              >
                <div className="flex items-center gap-2 text-sm">
                  <span className="p-1.5 bg-slate-200/70 rounded-lg text-slate-700 flex items-center justify-center">
                    <Store className="w-4 h-4" />
                  </span>
                  <span>散客 / 現場自取區</span>
                  <span className="text-xs bg-slate-200 text-slate-700 px-2 py-0.5 rounded-full font-medium">
                    {walkInOrders.length} 筆
                  </span>
                </div>
                <div className="flex items-center gap-1 text-slate-400 hover:text-slate-600 transition-colors">
                  <span className="text-xs font-normal">
                    {isWalkInAccordionOpen ? '收合' : '展開'}
                  </span>
                  {isWalkInAccordionOpen ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                </div>
              </div>

              <AnimatePresence>
                {isWalkInAccordionOpen && (
                  <motion.div
                    key="walkin-schedule-content"
                  ...
                  >
                    <div className="pt-3 mt-2 border-t border-slate-200/60 space-y-3">
